...
def res_block(conv_data, filter_s, name='resnet'):
    with tf.variable_scope(name):
        x=conv_data
        gz1=tf.layers.conv2d(inputs=conv_data,filters=filter_s, kernel_size=[4,4], strides=(1,1), padding='same' , name='rr1')
        ga1= tf.nn.relu(tf.contrib.layers.instance_norm(gz1))
        gz2=tf.layers.conv2d(inputs=ga1,filters=filter_s, kernel_size=[4,4], strides=(1,1), padding='same', name='rr2')
...
def generator(image_A, name='generator', reuse=False):       #image_A is 256*256*3
    with tf.variable_scope(name, reuse=reuse):
        gz1=tf.layers.conv2d(inputs=image_A,filters=32, kernel_size=[7,7], strides=(1,1), padding='same', name='c1')
        ga1= tf.nn.relu(tf.contrib.layers.instance_norm(gz1))
        gz2=tf.layers.conv2d(inputs=ga1,filters=64, kernel_size=[4,4], strides=(2,2), padding='same', name='c2')
        ga2=tf.nn.relu(tf.contrib.layers.instance_norm(gz2))
        gz3=tf.layers.conv2d(inputs=ga2,filters=128, kernel_size=[4,4], strides=(2,2), padding='same', name='c3')
        ...
        gz4=tf.layers.conv2d_transpose(inputs=res6,filters=64, kernel_size=[4,4], strides=(2,2), padding='same', name='c4')
        ga4= tf.nn.relu(tf.contrib.layers.instance_norm(gz4))
        gz5=tf.layers.conv2d_transpose(inputs=ga4,filters=32, kernel_size=[4,4], strides=(2,2), padding='same', name='c5')
        ga5=tf.nn.relu(tf.contrib.layers.instance_norm(gz5))
        gz6=tf.layers.conv2d_transpose(inputs=ga5,filters=3, kernel_size=[7,7], strides=(1,1), padding='same', name='c6')
...
def discriminator(image_B, name='discriminator', reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        dz1=tf.layers.conv2d(inputs=image_B,filters=64, kernel_size=[4,4], strides=(4,4), padding='same', name='c1')
        da1= tf.nn.leaky_relu(dz1,.2)
        dz2=tf.layers.conv2d(inputs=da1,filters=64, kernel_size=[4,4], strides=(2,2), padding='same', name='c2')
        da2= tf.nn.leaky_relu(tf.contrib.layers.instance_norm(dz2),.2)
        dz3=tf.layers.conv2d(inputs=da2,filters=128, kernel_size=[4,4], strides=(2,2), padding='same', name='c3')
        da3= tf.nn.leaky_relu(tf.contrib.layers.instance_norm(dz3),.2)
        dz4=tf.layers.conv2d(inputs=da3,filters=128, kernel_size=[4,4], strides=(2,2), padding='same', name='c4')
        da4= tf.nn.leaky_relu(tf.contrib.layers.instance_norm(dz4),.2)
        dz5=tf.layers.conv2d(inputs=da4,filters=256, kernel_size=[4,4], strides=(2,2), padding='same', name='c5')
        da5= tf.nn.leaky_relu(tf.contrib.layers.instance_norm(dz5),.2)
        dz6=tf.layers.conv2d(inputs=da4,filters=1, kernel_size=[4,4], strides=(1,1), padding='valid', name='c6')
        da6=tf.nn.sigmoid(dz6)
        output=da6
        return output
...
def main(job_dir,**args):
    ...
    DAloss=(DAloss1+DAloss2)/2
    DBloss=(DBloss1+DBloss2)/2
